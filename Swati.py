...
class PageTwo(tk.Frame): 
    ...
    def __init__(self, parent, controller):
        ...
        def Google_FirstLink():
            ...
            headers= {"User-Agent":#add user agent as required.              #changes required
                      ":"1"}
...
#             webbrowser.open(url, new=1, autoraise=True)
            ...
            headers= {"User-Agent":#add user agent            changes required
                      :"1"}
...
#                 ProductName =str(ProductName).split(' - ')[0]
            ...
            # image url
            try:
                #uncomment ProductImage_Url_src = str(soup.find('script', id="jsonLD")).split("http://rukmini1")[1].split('?')[0]
            except:
                try:
...
#                 SearchProduct = Feature_list[1]
            ...
            headers_c = {'User-Agent': #add user agent              changed required
                         :"1"}
...
#             webbrowser.open(url, new=1, autoraise=False)
        ...
        def FlipkartProductPage():
            ...
            def ProductCompare():
                ...
                # Product Name label
                ProductName_label = tk.Label(top2, text = ProductName, font=X_Font, bg="white" , wraplength= 500)
